## Author: Qin Qian
## Time-stamp: < modified by qinqianhappy :2012-12-10 19:21:34 >
## TCGA process
...
exp.patientclass <- function(patient.match.mutated,exp.match, toptable, mut.genes, cutoff){
  for(gene in mut.genes) {
      ...
      write.table(sort(t.p.value), file=paste("../results/ttest_colon", gene, "mutation.txt", collapse="", sep=""), quote=F, sep="\t", col.names = F)
      write.table(sort(w.p.value), file=paste("../results/wilcoxon_colon.txt", gene, "mutation.txt", collapse="", sep=""), quote=F, sep="\t", col.names = F)
      ...
      qqplot(t.p.value.unif, t.p.value, col=2, main=paste(gene,"qqplot t.test p value uniform"))
      ...
      qqplot(w.p.value.unif, w.p.value, col=2, main=paste(gene, "qqplot wilcoxon test p value uniform"))
      ...
      scatterplot(sort(t.p.value.unif),sort(t.p.value), main=paste("Colon t.test scatterplot",gene))
      scatterplot(sort(w.p.value.unif),sort(w.p.value), main=paste("Colon w.test scatterplot",gene))
      ## common scatterplot
      plot(sort(t.p.value.unif), sort(t.p.value),
           main=paste(gene, "common uniform p.value against observed p.value"), pch=2, col="blue", lty=3, lwd=1)
      ## linear regression
      ...
      plot(sort(w.p.value.unif), sort(w.p.value),
           main=paste(gene, "common uniform p.value against observed p.value"), pch=2, col="blue", lty=3)
      ## linear regression
...
toptable <- topN.class(15, Colon.Mutclass, mutation.table)
...
pdf("../results/Colon_Cancer_Scatter1211.pdf")
exp.patientclass(patient.match.mutated,exp.match, toptable, toptable$genes, 0.01)
dev.off()
...
## heatmap for differential expressed genes between two class patients
...
exon.explore <- function(exondata, Mutation.freq, cutoff){
  ...
  plot(sort(freq)~sort(length),data=mutation.exonlength, col="blue", main="original mutation frequency vs exon lengths")
...
## filter expression data overlapped with only mutated genes
